...
int main(void) {
    ...
    for (;;) {
        ...
        if (pid == -1) {
            perror("Shell programming error, unable to fork.\n");
        }
...
void process_input(int argc, char **argv) {
    ...
    for (int i = 0; i < argc; i++) {
        if (argv[i][0] == '<') {
            ...
            if (pipe(p) != 0) _exit_error("Not able to pipe.", true);
            argv[i] = NULL;
            pid = fork();
            if (pid == 0) {
                dup2(p[1], 1);
                close(p[0]);
                execvp(*argv, argv);
                _exit_error("Unable to run second command.", true);
            } else if (pid > 1) {
                argv = argv + i + 1;
                dup2(p[0], 0);
                close(p[1]);
                execvp(*argv, argv);
                _exit_error("Unable to run first command.", true);
            } else {
                _exit_error("Pipe fork error.", true);
            }
